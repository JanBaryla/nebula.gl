...
export default class Editor extends PureComponent<EditorProps, EditorState> {
  ...
  _addFeature = (mode: string, point: ScreenCoordinates) => {
    ...
    if (mode === MODES.DRAW_5MILE_CIRCLE || mode === MODES.DRAW_3MILE_CIRCLE || mode === MODES.DRAW_1MILE_CIRCLE) {
      const radius = mode === MODES.DRAW_5MILE_CIRCLE ? 8046.72 : (mode === MODES.DRAW_3MILE_CIRCLE ? 4828.032 : 1609.344);
      feature.isClosed = true;
      const polygon = circleToPolygon(this._unproject([point.x, point.y]), radius, 36);
      let newFirstPoint: Array<number> = [
